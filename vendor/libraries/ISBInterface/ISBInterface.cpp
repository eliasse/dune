#include "ISBInterface.h"
#include <iostream>
/*
ISBInterface::ISBInterface() {
  //Do something?
};
*/

//----------------------------------------------------------------
uint8_t ISBInterface::calc_checksum(char* buffer, uint8_t len) {
  uint8_t chk = 0;
  for (int ii = 0; ii < len; ii++) { chk = chk ^ buffer[ii]; } // XOR
  return chk;
}

bool ISBInterface::parse_data(char c) {
...
bool ISBInterface::parse_package() {
...
void ISBInterface::clear_package() {
  unpack_index = 0;
  package_available = false;
}


void ISBInterface::new_package(uint8_t _msgID) {
  send_ptr = send_buffer; //reset send buffer
  add_byte('#'); //Add start byte
  add_byte(_msgID);
}

bool ISBInterface::send_package() {
  //Calcualte length
...
//-----------------------Add data to package----------------------
//----------------------------------------------------------------
void ISBInterface::add_byte(uint8_t b) {
...
//----------------------------------------------------------------
void ISBInterface::add_float(float val){
...
//----------------------------------------------------------------
void ISBInterface::add_double(double val){
...
//----------------------------------------------------------------
void ISBInterface::add_long(uint32_t val){
...
//----------------------------------------------------------------
double ISBInterface::parse_double(){
...
//----------------------------------------------------------------
uint32_t  ISBInterface::parse_long(){
...
//----------------------------------------------------------------
uint64_t ISBInterface::parse_llong() {
