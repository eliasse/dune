...
#define utm_sa 6378137.000000
#define utm_sb 6356752.314245
#define utm_k0 0.9996
#define utm_pi 3.141592653589793

class UTM {
  private:
    double e2  = pow( pow(utm_sa,2) - pow(utm_sb,2) , 0.5 ) / utm_sb;
    double e2c = pow(e2,2);
    double c   = pow(utm_sa,2) / utm_sb;
...
void GeoToUTM(double lat_, double lon_){
  ...
  zone_n = (int) (lon_ * 180/utm_pi / 6 + 31);
  S = zone_n * 6 -183;
  deltaS = lon_ - S * utm_pi/180.0;

  lat_2 = lat_ * 180/utm_pi;
  ...
  v  =  c * utm_k0 /  pow( 1.0 + e2c * pow(cos(lat_), 2),0.5);
  ...
  Bm = utm_k0 * c * ( lat_ - alfa * j2 + beta * j4 - gama * j6 );
