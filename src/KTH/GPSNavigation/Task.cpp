...
namespace KTH
{
    ...
    namespace GPSNavigation
    {
        ...
        struct Task: public DUNE::Tasks::Task
        {
            ...
            //! Yaw entity eid.
            int m_yaw_eid;
            //! True heading from SBG
            int m_true_heading_eid;
            //! Time for last received true heading
            unsigned int m_last_true_heading = 0;
            //! Height offset.
            float m_offset;
            //! Offset flag.
            bool m_offset_flag;
                    ...
                    DUNE::Tasks::Task(name, ctx)
            {
                ...
                param("Entity Label - True Heading", m_args.elabel_true_heading)
                        .description("Entity label of True Heading");
            ...
            onEntityResolution(void)
            {
                ...
                try
                {
                    m_true_heading_eid = resolveEntity(m_args.elabel_true_heading);
                }
                catch (...)
                {
                    m_true_heading_eid = 0;
                }
            }
            ...
            consume(const IMC::EulerAngles* msg)
            {
                if (msg->getSourceEntity() == m_true_heading_eid)
                {
                    ...
                    m_last_true_heading = Time::Clock::getMsec();
                }

                unsigned long time_since_true_heading = Time::Clock::getMsec() - m_last_true_heading;

                if (msg->getSourceEntity() == m_imu_eid &&
                time_since_true_heading > 5000)
                {
                    m_estate.phi = msg->phi;
                    m_estate.theta = msg->theta;
                }
